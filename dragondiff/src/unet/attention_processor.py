...
class AttnProcessor(nn.Module):
    ...
    def __call__(
        ...
        if attn.updown == 'up' and not save_kv:
            if mode == "move":
                if iter_cur>=5: 
                    key_ref = attn.buffer_key[iter_cur].to('cuda:0', dtype=query.dtype)
                    value_ref = attn.buffer_value[iter_cur].to('cuda:0', dtype=query.dtype)
                    key = key_ref.repeat(2,1,1)
                    value = value_ref.repeat(2,1,1)
            elif mode == 'drag':
                if iter_cur>=5:
                    key_ref = attn.buffer_key[iter_cur].to('cuda:0', dtype=query.dtype).repeat(2,1,1)
                    value_ref = attn.buffer_value[iter_cur].to('cuda:0', dtype=query.dtype).repeat(2,1,1)
                ...
                if 35>=iter_cur>=0:
                    ...
                    mask_fg = (mask_fg>0.5).float().to('cuda:0', dtype=query.dtype)
                    ...
                    mask_bg = mask['base']
                    mask_bg = (mask_bg>0.5).float().to('cuda:0', dtype=query.dtype)
