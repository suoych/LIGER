...
from dragondiff.sam.efficient_sam.build_efficient_sam import build_efficient_sam_vits
from dragondiff.src.utils.utils import resize_numpy_image
...
def segment_only_with_points(
    image,
    global_points,
    global_point_label=[2,3]
):
    #global_points = [(0,0), (256,256)]
    if len(global_points) == 4:
        x1, y1, x2, y2 = global_points[0],global_points[1],global_points[2],global_points[3]
    else:  
        x1, y1 = global_points[0]
        x2, y2 = global_points[1]
    #x1, y1 = global_points[0]
...
def segment_only_with_points_paste(
    image_reference,
    global_points,
    image_base,
    dx=0, 
    ...
    pts_sampled = torch.reshape(torch.tensor(input_point), [1, 1, -1, 2])
    pts_labels = torch.reshape(torch.tensor(input_label), [1, 1, -1])
    img_tensor = transforms.ToTensor()(image_reference)
    # sam
    predicted_logits, predicted_iou = sam(
        img_tensor[None, ...],
        pts_sampled,
    ...
    return paste_with_mask_and_offset(image_reference, image_base, mask_uint8, dx, dy, resize_scale), mask_uint8
...
def get_point_move(original_image, img, sel_pix, evt):
    if original_image is not None:
