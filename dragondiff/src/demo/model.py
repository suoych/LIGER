from dragondiff.src.models.dragondiff import DragonPipeline
from dragondiff.src.utils.utils import resize_numpy_image, split_ldm, process_move, process_drag, process_appearance, process_paste
...
class DragonModels():
    ...
    def run_move(self, original_image, mask, mask_ref, prompt, resize_scale, w_edit, w_content, w_contrast, w_inpaint, seed, selected_points, guidance_scale, energy_scale, max_resolution, SDE_strength, ip_scale=None):
        ...
        dx = x_cur[0]-x[0]
        dy = y_cur[0]-y[0]
        #print(mask.shape,mask.min(),mask.max(),type(mask))
        #pdb.set_trace()
        ...
        # pre-process zT
        mask_tmp = (F.interpolate(img2tensor(mask)[0].unsqueeze(0).unsqueeze(0), (int(latent_in.shape[-2]*resize_scale), int(latent_in.shape[-1]*resize_scale)))>0).float().to('cuda:0', dtype=latent_in.dtype)
