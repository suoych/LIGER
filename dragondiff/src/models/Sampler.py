...
class Sampler(StableDiffusionPipeline):
    ...
    def edit(
        ...
        raw_tokens = text_input.input_ids.to(self.device)
        
        #raw_tokens.requires_grad=True
        text_embeddings = self.text_encoder(raw_tokens)[0]
        
        max_length = text_input.input_ids.shape[-1]
        uncond_input = self.tokenizer(
                [""], padding="max_length", max_length=max_length, return_tensors="pt"
            )
        uncond_raw_tokens = uncond_input.input_ids.to(self.device)

        uncond_embeddings = self.text_encoder(uncond_raw_tokens)[0]
        # image prompt
